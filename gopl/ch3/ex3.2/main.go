/*Exercício 3.2: Faça experimentos com visualizações de outras funções do
pacote math. Você pode gerar padrões como caixa de ovo, morrinhos
(moguls)1 ou uma sela?*/
...
func main() {
	...
	for i := 0; i < cells; i++ {
		for j := 0; j < cells; j++ {
			ax, ay, okA := corner(i+1, j, funcaoEscolhida)
			bx, by, okB := corner(i, j, funcaoEscolhida)
			cx, cy, okC := corner(i, j+1, funcaoEscolhida)
			dx, dy, okD := corner(i+1, j+1, funcaoEscolhida)
...
func corner(i, j int, f surfaceFunc) (float64, float64, bool) {
